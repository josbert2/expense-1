...
import { Copy, ExternalLink, Trash2, Receipt } from "lucide-react"

interface SharedSplitExpense {
  id: string
  expenseId: string
  expenseTitle: string
  url: string
...
interface SharedSplitExpensesTableProps {
  links: SharedSplitExpense[]
  onDelete: (id: string) => void
}

export function SharedSplitExpensesTable({ links, onDelete }: SharedSplitExpensesTableProps) {
  ...
  return (
    <div className="bg-gray-900/50 rounded-xl border border-gray-800/50 backdrop-blur-sm overflow-hidden">
      <div className="p-4 border-b border-gray-800/50">
        <h3 className="text-base font-medium text-white">Gastos Divididos Compartidos</h3>
      </div>
      ...
      {links.length === 0 ? (
        <div className="p-8 text-center">
          ...
          <p className="text-gray-500">No hay gastos divididos compartidos activos</p>
        </div>
      ) : (
          ...
          <Table>
            <TableHeader className="bg-gray-900/70">
              <TableRow className="border-b border-gray-800/50 hover:bg-transparent">
                <TableHead className="text-xs text-gray-400">Título</TableHead>
            ...
            <TableBody>
              {links.map((link) => (
                ...
                >
                  <TableCell className="text-sm font-medium text-white">{link.expenseTitle}</TableCell>
